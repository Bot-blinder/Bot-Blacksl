	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Vai na sombra, pq bosta no sol seca 😉😂, gosht do krl 😤 @${num.split('@')[0]}👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
		...
		try {
			...
			mess = {
				...
				only: {
					...
					ownerG: '📌 Este comando só pode ser usado pelo grupo do blinder! ',
					ownerB: '📌 Este comando só pode ser usado pelo dono do bot! ',
					admin: '📌 Este comando só pode ser usado por adm 😎👌! ',
			...
			switch(command) {
				...
				case 'Dono':
					...
					buffer = await getBuffer(`https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRmFqhv0gpgrKnWN9cDBzI4Ae_vhiUZ6XjjmQ&usqp=CAU`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*NOME:* BOT BLACK/n*WPP:* wa.me/+558192985915/n*INSTA:* edsonvsk_'})
					break
				/*case 'memeindo':
					memein = await kagApi.memeindo()
